...
/* MODES */
typedef enum { MODE_INTENSITY, MODE_COLOR, MODE_DEPTH } mode_t;
...
void handle_btn1(void)
{
    ...
    if(!now && btn1_prev)
    {
        ...
        {
            active_mode++;
            if(active_mode>MODE_DEPTH) active_mode=MODE_INTENSITY;
...
{
    if(!device_on || !btn(BTN2_GPIO)) return;

    ESP_LOGI("BTN2","INC");

    if(active_mode==MODE_INTENSITY && i_idx<5) i_idx++;
    else if(active_mode==MODE_COLOR && c_idx<2) c_idx++;
    else if(active_mode==MODE_DEPTH && d_idx==0) d_idx++;
...
{
    if(!device_on || !btn(BTN3_GPIO)) return;

    ESP_LOGI("BTN3","DEC");

    if(active_mode==MODE_INTENSITY && i_idx>0) i_idx--;
    else if(active_mode==MODE_COLOR && c_idx>0) c_idx--;
    else if(active_mode==MODE_DEPTH && d_idx==1) d_idx--;
